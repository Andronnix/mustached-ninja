...
function CLMenu(x, y, w, h, tempBar){
  this.toDo = function(){
    ...
    createjs.Sound.setVolume(maxVolume);
  ...
  this.text = new createjs.Text("Mustaged ninja", "bold small-caps 56px Arial", "#425752"); 
  this.text.x = 10 + WIDTH / 2 - this.text.getMeasuredWidth() / 2;
  this.text.y = HEIGHT / 3 + 10;
  this.text.alpha = 0;
  stage.addChild(this.text);
  shapeAppear(this.text);
...
function drawMenu(start){
  this.menu = ["Large City(hard)", "City 1 (no bonuses)", "City 2", "City 3"];
  ...
  else
  {
    ...
    var btn = new fadeBtn(WIDTH*0.375, this.currY, WIDTH/4, 50, fn, "Resume", false, this);
  ...
  for(var i = 0; i < this.menu.length; i++){
    ...
    var btn = new fadeBtn(WIDTH*0.375, this.currY, WIDTH/4, 50, fn, this.menu[i], start, this.menuMaps[i]);
    this.buttons.push(btn);
    this.currY += 70;
  }
  stage.update();
  ...
  this.L = createjs.Ticker.addEventListener("tick", function(){
    ...
    if (createjs.Sound.getVolume() <= 0){
      ...
      ME.L = createjs.Ticker.addEventListener("tick", function(){
        ...
        if (createjs.Sound.getVolume() >= maxVolume){
          createjs.Ticker.removeEventListener("tick", ME.L);
...
var volumeText = new createjs.Text("Volume 0%", "bold small-caps 36px Arial", "#425752"); 
volumeText.alpha = 0;
//volumeText.maxAlpha = 1;
volumeText.isSeen = false;

function volumeUp(x, y){
  createjs.Sound.setMute(false);
  if (createjs.Sound.getVolume() < 1)maxVolume+=0.1;
  createjs.Sound.setVolume(maxVolume);
  volumeText.maxAlpha=volumeText.alpha+4;
  if (!volumeText.isSeen){
    volumeText.AA = createjs.Ticker.addEventListener("tick", function(){
      volumeText.alpha += 0.025;
      console.log("volumeAlpha = " + volumeText.alpha);
      if (volumeText.alpha < 1)volumeText.y -= 0.1;
      if (volumeText.alpha >= volumeText.maxAlpha){
          volumeText.isSeen = false;
          volumeText.alpha = 0;
          var AA = volumeText.AA;
          createjs.Ticker.removeEventListener("tick", AA);
          stage.removeChild(volumeText);
        }
    });    
    stage.addChild(volumeText); 
    volumeText.isSeen = true;
    volumeText.y = HEIGHT / 1.5 + 50;
    volumeText.localToGlobal(volumeText.x, volumeText.y);
  }
  volumeText.text = "Volume " + Math.floor(maxVolume*100) + "%";
  volumeText.x = WIDTH / 2 - volumeText.getMeasuredWidth() / 2;
}

function volumeDown(x, y){
  createjs.Sound.setMute(false);
  if (createjs.Sound.getVolume() > 0.1)maxVolume-=0.1;
  createjs.Sound.setVolume(maxVolume);
  volumeText.maxAlpha=volumeText.alpha+4;
  if (!volumeText.isSeen){
    volumeText.AA = createjs.Ticker.addEventListener("tick", function(){
